...
/* ── AI Root Cause Analysis Panel ─────────────────────────────────────────
   ...
   - CEM anomalies (subscriber-level, with experience impact)
...
function buildRCA(sla: RCAProps["sla"], oss: any[], cem: any[], corrs: any[]) {
    ...
    const regions = [...new Set([
        ...
        ...criticalCem.map((a) => a.region).filter(Boolean),
    ])];
    ...
    const operators = [...new Set(criticalCem.map((a) => a.operator).filter(Boolean))];
    ...
    // 3. CEM impact
    if (criticalCem.length > 0) {
        findings.push({
            icon: "\uD83D\uDCB3",
            severity: "critical",
            title: `${criticalCem.length} high-impact CEM anomalies detected`,
            detail: `${operators.length > 0 ? `Operators affected: ${operators.join(", ")}. ` : ""}${criticalCem.filter((b) => b.line_type === "postpaid").length} postpaid + ${criticalCem.filter((b) => b.line_type === "prepaid").length} prepaid subscribers impacted.`,
        });
    }
    ...
    if (criticalOss.length > 0 && criticalCem.length > 0 && strongCorrs.length > 0) {
        ...
        narrative = `All systems operating within normal parameters. No critical anomalies detected across OSS or CEM domains. SLA compliance is maintained. The L4 agent continues passive monitoring.`;
    } else {
        narrative = `Mixed signals detected across the network. ${criticalOss.length + warningOss.length} OSS anomalies and ${criticalCem.length} CEM anomalies are being monitored. SLA risk at ${(score * 100).toFixed(1)}% — continued observation recommended.`;
    }

    return { riskLevel, score, findings, narrative, topDrivers, strongCorrs, criticalOss, criticalCem, regions };
}
...
/* ── Component ────────────────────────────────────────────────────────────── */
export default function RootCauseAnalysis({ sla, ossAnomalies, cemAnomalies, correlations }: RCAProps) {
    const rca = buildRCA(sla, ossAnomalies, cemAnomalies, correlations);
