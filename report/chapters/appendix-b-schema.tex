...
All result tables (\texttt{anomalies}, \texttt{sla\_risk\_scores}, \texttt{correlation\_insights}, \texttt{revenue\_anomalies}) reference \texttt{pipeline\_runs(run\_id)} as a foreign key. This enables:

\begin{itemize}
  \item \textbf{Run-level traceability:} Every result can be traced to the exact pipeline execution that produced it.
  \item \textbf{Temporal queries:} Results can be filtered by pipeline run timestamp.
  \item \textbf{Cascade behaviour:} \texttt{ON DELETE SET NULL} --- if a pipeline run is deleted, results are preserved but orphaned.
\end{itemize}

The \texttt{dataset\_registry} table links datasets to runs and tracks their location in the data lake (\texttt{object\_key}), enabling full data lineage from raw ingestion through curated output.

The \texttt{model\_registry} table records model versions and artifact locations, supporting model version tracking and rollback.
