...
def get_subscriber(imsi_hash: str, month_year: str = Query(default="2026-03"), user=Depends(require_auth)):
    with _db() as conn:
...
def get_area_cem(area: str, month_year: str = Query(default="2026-03"), user=Depends(require_auth)):
    with _db() as conn:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute("""
