...
def _apply_drift(
    df: pd.DataFrame, rat: str, drift_f: float, rng: np.random.Generator
) -> pd.DataFrame:
    ...
    if integ_c is not None:
        ...
        integ_drifted = np.clip(
            integ - drift_f * 1.5 + rng.normal(0, 0.4, len(df)), 80, 100
        )
        df[integ_c] = [f"{v:.2f}%" for v in integ_drifted]

    # CDR-like columns (call drop, BLER) — drift UP with load
    cdr_cols = [
        c for c in df.columns if "call drop" in c.lower() or "opt_npm" in c.lower()
    ]
    for c in cdr_cols:
        cdr = pd.to_numeric(df[c], errors="coerce").fillna(0.0)
        cdr_drifted = np.clip(
            cdr * (1 + drift_f * 1.2) + rng.normal(0, 0.05, len(df)), 0, 30
        )
    ...
    tput_cols = [
        c
        for c in df.columns
        if "throughput" in c.lower() or "hsdpa" in c.lower() or "thp" in c.lower()
    ]
    for c in tput_cols:
    ...
    user_cols = [
        c
        for c in df.columns
        if "user.avg" in c.lower() or "user.max" in c.lower() or "active" in c.lower()
    ]
    for c in user_cols:
        v = pd.to_numeric(df[c], errors="coerce").fillna(0.0)
        ...
        # Users are integers
        df[c] = (
            np.round(v_drifted).astype("Int64")
            if v.dtype.kind == "i"
            else np.round(v_drifted, 2)
        )
...
def main():
    ...
    print("Memory-bounded: sampling up to 300K rows per RAT into reservoir.\n")
    ...
    for rat in ("2G", "3G", "4G"):
        ...
        for token, (ym, drift_f) in MONTHS.items():
            ...
            total += 1
            print(
                f"    ✓ {out.name:35s} month={ym} drift={drift_f:+.2f} size={out.stat().st_size / 1e6:.1f} MB"
            )
