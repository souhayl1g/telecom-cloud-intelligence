...
def bootstrap_month(
    reservoir: pd.DataFrame, month_year: str, drift: float
) -> list[tuple]:
        ...
        if col in sampled.columns:
            if col in ["throughput_mbps", "active_users", "rsrp_dbm"]:
                ...
                sampled[col] = sampled[col] * (
                    1 - drift
                )  # inverse: worse network = lower quality
    ...
    timestamps = [
        base_date
        + timedelta(
            days=int(rng.integers(0, days_in_month)), hours=int(rng.integers(0, 24))
        )
        for _ in range(n)
    ]
    ...
    sampled["anomaly_flag"] = (sampled["integrity"] < 100.0) | (
    ...
    for _, row in sampled.iterrows():
        rows.append(
            (
                ...
                row["area"],
                row["month_year"],
                round(row["throughput_mbps"], 2)
                if pd.notna(row["throughput_mbps"])
                else None,
                round(row["latency_ms"], 2) if pd.notna(row["latency_ms"]) else None,
                round(row["packet_loss_rate"], 4)
                if pd.notna(row["packet_loss_rate"])
                else None,
                ...
                round(row["cell_load_pct"], 2)
                if pd.notna(row["cell_load_pct"])
                else None,
                bool(row["anomaly_flag"]),
                row["rat_type"],
                round(row["integrity"], 2) if pd.notna(row["integrity"]) else None,
                round(row["call_drop_rate"], 4)
                if pd.notna(row["call_drop_rate"])
                else None,
    ...
    print(
        f"  ... {len(rows):,} rows generated, anomaly rate: {sampled['anomaly_flag'].mean() * 100:.1f}%"
    )
    return rows
