...
def compute_oss_features(records: list[dict]) -> dict:
    ...
    if not records:
        return {
            "mean_throughput_mbps": 0.0, "std_throughput_mbps": 0.0,
            "mean_latency_ms": 0.0, "std_latency_ms": 0.0, "max_latency_ms": 0.0,
            "mean_packet_loss_pct": 0.0, "max_packet_loss_pct": 0.0,
            "mean_active_users": 0.0, "mean_signal_rsrp_dbm": 0.0,
        }
    tput = np.array([v for v in (r.get("throughput_mbps") for r in records) if v is not None], dtype=float)
    lat = np.array([v for v in (r.get("latency_ms") for r in records) if v is not None], dtype=float)
    loss = np.array([v for v in (r.get("packet_loss_pct", r.get("packet_loss_rate")) for r in records) if v is not None], dtype=float)
    usr = np.array([v for v in (r.get("active_users") for r in records) if v is not None], dtype=float)
    rsrp = np.array([v for v in (r.get("signal_rsrp_dbm", r.get("rsrp_dbm")) for r in records) if v is not None], dtype=float)
    return {
...
def compute_bss_features(records: list[dict]) -> dict:
    ...
    if not records:
        return {
            "mean_dou_total": 0.0, "mean_duration": 0.0,
            "mean_s1_mme_sr": 0.0, "mean_iu_attach_sr": 0.0,
            "mean_gb_attach_sr": 0.0, "mean_network_experience_index": 0.0,
        }
