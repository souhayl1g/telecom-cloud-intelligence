...
class NetworkAgent(BaseAgent):
    ...
    async def handle(self, intent: AgentIntent) -> AgentResult:
        ...
        if action == "monitor_cells":
            ...
            return self._correlate_with_cem(
                params.get("area"), params.get("month_year", "2026-03")
            )
        elif action == "area_health":
            return self._area_health(
                params.get("area"), params.get("month_year", "2026-03")
            )
        else:
            return AgentResult(
    ...
    def _monitor_cells(self, area: str, month_year: str) -> AgentResult:
        ...
        return AgentResult(
            ...
            success=True,
            data={"cells": cells, "anomaly_count": anomaly_count},
            summary=summary,
        )
    ...
    def _detect_anomaly(self, area: str, month_year: str) -> AgentResult:
        ...
        if not rows:
            return AgentResult(
                agent_name=self.name, success=False, error="No cells found"
            )
        ...
        summary = (
            f"Detected {len(anomalies)} anomalous cells in {area} for {month_year}"
        )
        return AgentResult(
            agent_name=self.name,
            success=True,
    ...
    def _capacity_forecast(self, area: str, month_year: str) -> AgentResult:
        ...
        recommendation = (
            ...
            if max_load > 90
            else "Plan capacity within 30 days"
            if max_load > 75
            else "Capacity healthy"
        )
    ...
    def _correlate_with_cem(self, area: str, month_year: str) -> AgentResult:
        ...
        if not row:
            return AgentResult(
                agent_name=self.name, success=False, error="No area health data"
            )
        ...
        if row["underserved_pct"] > 20:
            issues.append(
                "Large underserved population suggests network-device mismatch"
            )
        ...
        return AgentResult(
            ...
            summary=f"{area}: CEM={row['avg_cem_score']:.2f}, {row['underserved_pct']:.1f}% underserved. "
            + " ".join(issues),
        )
